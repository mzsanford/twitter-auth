...
describe TwitterAuth::Dispatcher::Basic do
  ...
  describe '#request' do
    before do
      ...
      FakeWeb.register_uri(:any, 'https://twitterman:test@twitter.com:443/fake.json', :body => {'fake' => true}.to_json)
        FakeWeb.register_uri(:get, 'https://twitterman:test@twitter.com:443/fake.xml', :body => '<fake>true</fake>')
    end
    ...
    it "should raise a TwitterAuth::Dispatcher::Error if response code isn't 200" do
      FakeWeb.register_uri(:get, 'https://twitterman:test@twitter.com:443/bad_response.json', :body => {'error' => 'bad response'}.to_json, :status => ['401', 'Unauthorized'])
    ...
    it 'should set the error message to the JSON message' do
      FakeWeb.register_uri(:get, 'https://twitterman:test@twitter.com:443/bad_response.json', :body => {'error' => 'bad response'}.to_json, :status => ['403', 'Forbidden'])
    ...
    it 'should raise a TwitterAuth::Dispatcher::Unauthorized on 401' do
      FakeWeb.register_uri(:get, 'https://twitterman:test@twitter.com:443/unauthenticated_response.xml', :body => "<hash>\n<request>/unauthenticated_response.xml</request>\n<error>bad response</error>\n</hash>", :status => ['401', 'Unauthorized'])
    ...
    it 'should set the error message to the XML message' do
      FakeWeb.register_uri(:get, 'https://twitterman:test@twitter.com:443/bad_response.xml', :body => "<hash>\n<request>/bad_response.xml</request>\n<error>bad response</error>\n</hash>", :status => ['403', 'Forbidden'])
      lambda{@dispatcher.request(:get, '/bad_response')}.should raise_error(TwitterAuth::Dispatcher::Error, 'bad response')
    end
  end
